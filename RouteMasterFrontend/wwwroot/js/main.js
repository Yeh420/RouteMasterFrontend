(function ($) {
	...
	$(document).ready(async function () {
		const datas = await $.ajax({
			url: 'https://localhost:7251/api/RoomProducts/1',
			method: 'Get'
		});
		...
		var dateArr = $.map(datas, function (data) {
			return formatDate(data.date.substring(0, 10));
		});
			...
			for (var cell = 0; cell < 42 - r; cell++) { // 42 date-cells in calendar
				if (cell >= calMonthArray.length) {
					...
					if (
						...
						var m = checkToday(iter_date) ? '<div class="today"' : '<div class="future-date"';
					}

					//calendar.datesBody.append(`${m}><span>${shownDate}</span><br/><span class="rpPrice"></span></div>`);

					var dateValue = `${passed_year}-${(passed_month + 1)}-${shownDate}`;
					var index = dateArr.indexOf(dateValue);

					if (index !== -1) {
						//calendar.datesBody.find('.rpPrice').text(`${datas[index].newPrice}`);
						calendar.datesBody.append(`${m}><span>${shownDate}</span><br/><span class="rpPrice">${datas[index].newPrice}</span></div>`);
					}
					//else {
					//	calendar.datesBody.append(`${m}><span>${shownDate}</span><br/><span class="rpPrice"></span></div>`);
					//}
